...
def evaluation_matrix(fname,cur_l_per):
    f=open(fname,'r')
    ...
    print ' {:12s}| {:8d}| {:8d}| {:8d}| {:8d}| {:5.7f}| {:4.7f}| {:4.7f} | {:4.7f}|'.format(temp_item,tp,fp,tn,fn,precision,recall,rand_index,jc)
...
def main():
    ...
    # Form file list
    #base_dir = sys.argv[1]
    result_dir = '%s/all_words' %('results')
    l= [0.02,0.04,0.06]
    #l=[0.2,0.2,0.2]
    ...
    print ' {:13s} | {:7s} | {:7s} | {:7s} | {:7s} | {:9s}| {:9s}| {:9s}| {:10s} |'.format('PER_LOC_ORG', 'TP','FP','TN','FN','Precision','Recall','Rand_index','Jaccard_Coefficient')
