...
def main():
    ...
    for dataset in dataset_list[0:3]:
        ...
        for setup in setup_list[0:2]:
            ...
            df = pd.DataFrame(result_list, columns=['fname', 'time','topic', 'alpha', 'eta_1', 'eta_2', 'tp', 'tn', 'fn', 'fp', 'precision', 'recall',
                                                    'f_measure' , 'rand_index', 'jc','accuracy', 'iteration_count'])
            df.sort_values([ 'topic', 'alpha', 'eta_1', 'eta_2'], inplace=True)
            fout_name_total = os.path.join(out_dir, '_'.join( [dataset, setup])+ '.xlsx')
            df.to_excel( fout_name_total,  sheet_name=setup, index=False, startrow=0, startcol=0)
